    ...
    with torch.no_grad():
        for batch_idx, (X, y) in enumerate(cifar10_dataloader):
            if batch_idx >= batch_num:
                ...
                wandb.log({"_Batch_{batch}_{datasource}_X.jpg".format(batch=batch_idx, datasource=datasource): [
                    wandb.Image(grid_X)]}, commit=False)
                grid_Xi = torchvision.utils.make_grid(xi.data, nrow=8, padding=2, normalize=True)
                wandb.log({"_Batch_{batch}_{datasource}_Xi.jpg".format(batch=batch_idx, datasource=datasource): [
                    wandb.Image(grid_Xi)]}, commit=False)
                grid_X_Xi = torchvision.utils.make_grid((X - xi).data, nrow=8, padding=2, normalize=True)
                wandb.log({"_Batch_{batch}_{datasource}_X-Xi.jpg".format(batch=batch_idx, datasource=datasource): [
