...
def main_worker(gpu, ngpus_per_node, args):
    ...
    ], lr=learning_rate, betas=(0.5, 0.999), weight_decay=1.e-6)
    ...
    train_set = folder.ImageFolder(root=traindir, transform=transform_train,
                                   label_mapping=label_map)
    n_train = len(train_set)

    test_set = folder.ImageFolder(root=valdir, transform=transform_test,
                              label_mapping=label_map)
    if args.distributed:
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=args.epochs * np.ceil(n_train / args.batch_size),
...
def train(train_loader, model, optimizer, epoch, args):
    ...
    loss_rec = AverageMeter('LossRec', ':.4e')
    loss_ce = AverageMeter('LossCe', ':.4e')
    loss_entropy = AverageMeter('LossEntropy', ':.4e')
    loss_kl = AverageMeter('LossKl', ':.4e')
    top1 = AverageMeter('Acc@1', ':6.2f')

    progress = ProgressMeter(
        len(train_loader),
        [batch_time, data_time, losses, loss_rec, loss_ce, loss_entropy, loss_kl, top1],
    ...
    for i, (x, y) in enumerate(train_loader):
        ...
        if torch.cuda.is_available():
            ...
            y_b = y_b.cuda(args.gpu, non_blocking=True)
            bs = x.size(0)
        # compute output
...
def validate(val_loader, model,  args):
    batch_time = AverageMeter('Time', ':6.3f')
    top1 = AverageMeter('Acc@1', ':6.2f')
    top1_x_xi = AverageMeter('Acc@1', ':6.2f')
    top1_xi = AverageMeter('Acc@1', ':6.2f')
    acc_avg = AverageMeter('Acc@1', ':6.2f')
    sparse_avg = AverageMeter('Acc@1', ':6.2f')
    TC = AverageMeter('Acc@1', ':6.2f')
