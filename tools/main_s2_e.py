...
parser.add_argument('--save_dir', default='./results/autoaug_s2_e/', type=str, help='save_dir')
parser.add_argument('--net_type', default='wide-resnet', type=str, help='model')
parser.add_argument('--depth', default=28, type=int, help='depth of model')
parser.add_argument('--ad_path', default='./results/autoaug_0207/model_new.pth', type=str, help='ad_path')
parser.add_argument('--ad_beta', default=0.5, type=float, help='noise scale')
...
net.apply(conv_init)
model = CVAE_s2_e(d=CNN_embed_dim, beta=args.ad_beta)
model_dict =  model.state_dict()
save_model = torch.load(args.ad_path)
...
def train(epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        optimizer.zero_grad()
        inputs, targets = Variable(inputs), Variable(targets)
        augoutputs, _ = model(inputs)
        outputs = net(augoutputs)               # Forward Propagation
...
def test(epoch):
    ...
    with torch.no_grad():
        ...
        print("\n| Validation Epoch #%d\t\t\tLoss: %.4f Acc2@1: %.2f%%" %(epoch, loss2.item(), acc2))
        writer.add_scalar('test/acc', acc, epoch)
        writer.add_scalar('test/acc2', acc2, epoch)
        if acc > best_acc:
